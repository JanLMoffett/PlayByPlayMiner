...
class ActionUnit:
    ...
    #--------------------------------------------------------------------------
    def __init__(self, action_unit_string, place_in_seq, away_batting_order, home_batting_order):
...
def main():    
    ...
    for i in range(len(away_pbps)):
        ...
        pbp1 = PlayByPlay(away_pbps[0])
        a0au = pbp1.get_action_units()
    ...
    print("Home Play by Play String 0:")
    print(home_pbps[0])
    print()
    pbp2 = PlayByPlay(home_pbps[0])
    h0au = pbp2.get_action_units()
    ...
    header1 = ["Seq","Name","Team","Type","BIP","BIP Location","Hit","Hit Location","Field Out","Out Location","On Base","Base Batter On"]
    csv_print(header1)
    for j in range(len(a0au)):
        ...
        tau.assign_player()
        bip = tau.is_bip()
        bip_loc = tau.get_bip_location()
        hit = tau.is_hit()
        hit_loc = tau.get_hit_location()
        field_out = tau.is_field_out()
        out_loc = tau.get_out_location()
        on_base = tau.is_on_base()
        base = tau.get_base()
        tl = [tau.place_in_seq, tau.au_player, tau.au_team, tau.au_type, bip, bip_loc, hit, hit_loc, field_out, out_loc, on_base, base]
