...
class BatUnit:
    ...
    fo_locations = ["to p", "to c", "to 1b", "to 2b", "to 3b", "to ss", "to lf", "to cf", "to rf"]
    ...
    #--------------------------------------------------------------------------
    def __init__(self, bat_unit_str, team, batting_order, bo_object):
        self.unit_string = bat_unit_str
        self.team = team
        self.bo = batting_order
        self.baseout = bo_object
    ...
    def get_field_out(self):
        ts = self.unit_string
        fol = "" #field out location
        fot = "" #field out type
        foq = "" #quality of batted ball
    ...
    def update_bo(self):
        if self.basesadded == 1:
            self.baseout.set_on_first(self.batter_name)
            self.baseout.set_on_second("NA") #clear bases ahead of current runner, they will be updated in br class
            self.baseout.set_on_third("NA")
            
        elif self.basesadded == 2:    
            self.baseout.set_on_second(self.batter_name)
            self.baseout.set_on_third("NA")
        
        elif self.basesadded == 3:
            self.baseout.set_on_third(self.batter_name)
            
        elif self.bassesadded == 4:
            self.baseout.set_on_first("NA")
            self.baseout.set_on_second("NA")
            self.baseout.set_on_third("NA")
...
class BRUnit:
    def __init__(self, br_unit_str, team, batting_order, bo_object):
    ...
    def update_bo(self):
        ...
        if stb != enb: #the baserunner advanced or was out
            if enb == 2:
                self.baseout.set_on_second(self.runner_name)
                self.baseout.set_on_third("NA") #clear bases that are ahead of current runner
            elif enb == 3:
                self.baseout.set_on_third(self.runner_name)
...
class SubUnit:
    def __init__(self, sub_unit_str, team, batting_order, subs):
        ...
        if self.to_i == -1:
            self.to_i = sub_unit_str.rfind("pinch",0,self.for_i)
            if "pinch hit" in sub_unit_str:
                self.sub_pos = "ph"
            elif "pinch ran" in sub_unit_str:
                self.sub_pos = "pr"
        self.player_in = StdzNames.stdz_name(self.unit_string[0:self.to_i-1])
        self.player_out = StdzNames.stdz_name(self.unit_string[self.for_i+4:])
    ...
    def mod_batting_order(self):
        ...
        if self.sub_pos != "p": #if substitution isn't for pitcher
            
            sub_hole = 0
            pout = self.player_out
            pin = self.player_in
            
            #find where in which batting order player out is
            #build new list with player out replaced by player in
            
            tbo = self.bo
            for i,s in enumerate(tbo):
                if s == pout:
                    sub_hole = i+1
                    tbo[i] = pin
            self.bo = tbo
            
            #return sub place in batting order
            return sub_hole
        else:
            return 0
#//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//  
